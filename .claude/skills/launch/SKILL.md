...
`$ARGUMENTS` is a space-separated list of issue numbers (with or without `#`). Example: `/launch #283 #284 285`
...
1. `git fetch origin`
2. Fetch issue title: `gh issue view <N> --repo langwatch/scenario --json title --jq '.title'`
...
8. Copy `.env*` files from the repo root into the worktree root (if any exist)

### 3. Create tmux sessions

For each worktree:
- Session name = directory basename (e.g., `issue283-support-optional-run-id`)
