 ...
 */
export interface VoiceJudgeAgentConfig extends TestingAgentConfig {
  /**
...
function buildContinueTestTool(): ChatCompletionTool {
  return {
    type: "function",
    function: {
      name: "continue_test",
...
function buildFinishTestTool(criteria: string[]): ChatCompletionTool {
 ...
 */
class VoiceJudgeAgent extends JudgeAgentAdapter {
  private openai = new OpenAI();
  private logger = new Logger("JudgeAgent");
  role: AgentRole = AgentRole.JUDGE;
  criteria: string[];

  constructor(private readonly cfg: VoiceJudgeAgentConfig) {
    super();
  ...
  async call(input: AgentInput) {
    ...
    const tools: ChatCompletionTool[] = [
      buildContinueTestTool(),
      buildFinishTestTool(cfg.criteria),
    ];
    ...
    if (completion.choices[0].message?.tool_calls?.length) {
      ...
      switch (toolCall.function.name) {
        case "finish_test": {
          args = JSON.parse(toolCall.function.arguments) as FinishTestArgs;
        ...
        default:
          return {
            success: false,
            messages: input.messages,
            reasoning: `JudgeAgent: Unknown tool call: ${toolCall.function.name}`,
            metCriteria: [],
  ...
  private async generateText(input: {
 ...
 * ```
 */
export const voiceJudgeAgent = (cfg: VoiceJudgeAgentConfig) => {
  return new VoiceJudgeAgent(cfg);
};
