...
export interface TtsConfig {
   ...
   * many styles. Mapping is provider-specific — ElevenLabs prepends an inline
   * `[angry]` marker, OpenAI passes it as `instructions`; see the provider
   * leaves under `voice/tts/`. Lowest-precedence source: the simulator's own
   * `voiceStyle` and a per-step `user("…", { voiceStyle })` override both win
   * over it.
   */
  voiceStyle?: string;
  /** Output format hint. */
