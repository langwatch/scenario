...
describe('convertCoreMessagesToAguiMessages', () => {
  it('converts a system message', () => {
    ...
    expect(result).toEqual([
      { id: 'core-id', role: 'system', content: 'sys' },
    ]);
  });
  ...
  it('converts a user message with string content', () => {
    ...
    const result = convertCoreMessagesToAguiMessages(input);
    expect(result).toEqual([
      { id: 'core-id', role: 'user', content: 'hello' },
    ]);
  });
  ...
  it('converts an assistant message with array content (multiple non-text)', () => {
    ...
    const input = [makeCoreMessage({ role: 'assistant', content: arr })];
    const result = convertCoreMessagesToAguiMessages(input);
    expect(result[0].content).toBe(JSON.stringify([{ type: 'json', value: { bar: 2 } }]));
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  it('converts a tool message with multiple parts', () => {
    ...
    const result = convertCoreMessagesToAguiMessages(input);
    expect(result).toEqual([
      { id: 'core-id-0', role: 'tool', toolCallId: 't1', content: JSON.stringify({ foo: 'bar' }) },
      { id: 'core-id-1', role: 'tool', toolCallId: 't2', content: JSON.stringify({ baz: 42 }) },
    ]);
  });
