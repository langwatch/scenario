...
| H3-vendor | + per-procedure BLOCKING gate ≡ judge | *(in flight)* | the load-bearing test: does the gate FINALLY fire (block the dropped steps → force completion)? |

This is the first scenario with real HEADROOM: baseline misses via genuine adherence failures the subject HAD the procedures for. If H3's per-procedure gate blocks the dropped steps and forces completion (de-risk Case A proved the mechanism; on refund the gate never fired because the subject never missed), H3 > baseline here would be the FIRST genuine evidence that per-procedure enforcement is load-bearing for the goal. Result logged on completion.
