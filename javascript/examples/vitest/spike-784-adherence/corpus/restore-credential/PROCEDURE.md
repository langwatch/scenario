...
kind: procedure
keywords: [restore, credential, audited, procedure, runbook]
links: [escalate-ticket, snapshot-endpoint, throttle-service, archive-report]
status: active
---
# Restore Credential

## Purpose
This procedure describes how to recover credential from a known-good copy. It exists so the operation is performed the same, safe way every time, regardless of who runs it, and so that a fresh operator can carry it out from the written steps alone.
...
## Procedure
1. Select the recovery point for credential.
2. Restore the expiry timestamp into the identity provider.
3. Verify the validation check.
4. Reconcile any gap.
