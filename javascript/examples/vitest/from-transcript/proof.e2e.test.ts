/**
 * #779 — END-TO-END proof (LIVE model calls, not mocked).
 *
 ...
 * model (OpenAI by default; SC779_PROVIDER=gemini for Gemini) seeded with the captured
 * *conversation*; we do NOT reconstruct Claude Code's real system prompt (that's the
 * deferred sandbox path, Strategy D). Proof B's point is that the omitted config flips the outcome.
 *
 * Gated behind SC779_LIVE (skipped by default so CI's example suite needs no funded key).
 * Run:  cd javascript/examples/vitest && SC779_LIVE=1 pnpm exec vitest run from-transcript/proof.e2e.test.ts
 * Needs OPENAI_API_KEY, or SC779_PROVIDER=gemini + GEMINI_API_KEY (auto-loaded from .env by the vitest setup).
 */
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
import scenario, { AgentRole, type AgentAdapter, type ScenarioResult } from "@langwatch/scenario";
import { generateText, type ModelMessage } from "ai";
import { describe, it, expect } from "vitest";
import { fromTranscript, type SeededScenario } from "./from-transcript";
import { e2eModel, E2E_MODEL_LABEL } from "./model";
...
function printSeed(label: string, b: SeededScenario) {
  ...
  console.log(
    `seed: ${b.stats.seededMessages} messages  ` +
      `(from ${b.stats.parsedNodes} parsed nodes → ${b.stats.chainLength}-node parentUuid chain → ` +
  ...
  if (b.stats.droppedByMatch) console.log(`   ⚠ scrubbed ${b.stats.droppedByMatch} token-bearing message(s) to reproduce the 'absent memory' failure`);
...
function printVerdict(result: ScenarioResult, reply: string) {
...
// Gated behind SC779_LIVE (skipped by default) so CI's example suite does not require a funded
// model key. Reproduce this PR's run with:  SC779_LIVE=1 SC779_PROVIDER=gemini pnpm exec vitest run …
describe.skipIf(!process.env.SC779_LIVE)("#779 — Scenario from a real Claude Code transcript (live)", () => {
  it(
    "Proof A (DoD-3 + DoD-4): faithful full-history replay runs to a judge verdict; >10-msg seed survives maxTurns",
    async () => {
      const b = fromTranscript(FIXTURE, { flattenTools: true });
      printSeed("PROOF A — faithful replay (token in context)", b);
      console.log(`   model: ${E2E_MODEL_LABEL}`);
      ...
      const result = await scenario.run({
        ...
        agents: [agent, scenario.judgeAgent({ criteria: CRITERIA_RECALL, model: e2eModel() })],
        script: [b.seedStep, scenario.agent(), scenario.judge()],
        setId: "779-from-transcript",
      });
  ...
  it(
    ...
    async () => {
      ...
      let naive: ScenarioResult | null = null;
      try {
        naive = await scenario.run({
          ...
          maxTurns: MAXTURNS,
          agents: [liveAgent(HARNESS_PREAMBLE), scenario.userSimulatorAgent({ model: e2eModel() }), scenario.judgeAgent({ criteria: CRITERIA_RECALL, model: e2eModel() })],
  ...
  it(
    ...
    async () => {
      ...
      const b = fromTranscript(FIXTURE, { flattenTools: true, dropMatching: /KUMQUAT77/i });
      ...
      const before = await scenario.run({
        ...
        maxTurns: 10,
        agents: [baseAgent, scenario.judgeAgent({ criteria: CRITERIA_RECALL, model: e2eModel() })],
        script: [b.seedStep, scenario.agent(), scenario.judge()],
        setId: "779-from-transcript",
      });
      ...
      const after = await scenario.run({
        ...
        maxTurns: 10,
        agents: [fixedAgent, scenario.judgeAgent({ criteria: CRITERIA_RECALL, model: e2eModel() })],
        script: [b.seedStep, scenario.agent(), scenario.judge()],
        setId: "779-from-transcript",
      });
