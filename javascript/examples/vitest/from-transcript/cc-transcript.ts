/**
 * #779 — Claude Code transcript reader + Anthropic→AI-SDK message adapter.
 *
 * Strategy B (adapter-specific, JSONL-sourced). This is the CC-adapter knowledge:
 ...
 */
/** A NormalizedTurn during construction, carrying the private same-message.id merge tag. */
type BuildTurn = NormalizedTurn & { _msgId?: string };
...
export function normalize(chain: RawNode[]): NormalizedTurn[] {
  ...
  for (const n of chain) {
    ...
    if (n.type === "assistant") {
      ...
      if (prev && prev.role === "assistant" && msgId && prev._msgId === msgId) {
