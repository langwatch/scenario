...
- **A complete trace then has to hold still.** A leaf span, a tool call with no children, leaves no unresolved parent behind when it is late, so a trace can look complete one poll before its last span arrives. The judge keeps polling until the set of fetched spans is unchanged for a quiet period, 2 seconds by default in the JavaScript SDK (`traceQuietPeriodMs`, also settable project-wide). This costs about 2 seconds per verdict; set it to `0` to settle on the first complete poll. If the wait budget runs out while the quiet period is still running, the trace settles cleanly anyway, with no error span.
