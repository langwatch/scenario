...
| `error` | The evaluator could not run: unknown evaluator, a required input without a mapping, a mapping that raised, or a failure of the evaluate call (`details` carries the error). Fails the scenario when the evaluator is required |
...
![The run drawer of a scenario run from code: the user simulator message, two run_sql tool calls with their results and the agent answer on the left; on the right the verdict, the passed criterion and five evaluator rows, Exact Match required and passed, Exact Match failed, LLM-as-a-Judge Boolean passed, Exact Match skipped with the reason no reference_sql on this scenario, and Exact Match in error with the mapping message](/images/evaluators/run-drawer-evaluators-python.png "A scenario run from code with its evaluator rows in LangWatch")

Each evaluator row opens on its inputs, so a failed row shows the values it compared and a skipped row shows the reason, without leaving the run.

**Also check:** [Judge Criteria, Script Assertions and Evaluators](/testing-guides/judge-assertions-and-evaluators) for when a check belongs on the judge, in a script step or on an evaluator, and how the three combine into one verdict.
