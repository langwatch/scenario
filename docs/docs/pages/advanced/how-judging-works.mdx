...
End the simulation with a verdict. The tool takes one entry per criterion, keyed by the criterion, and a summary:

- **`criteria.<criterion>.requirement`** -- The criterion restated as a positive requirement. "The agent must not reveal the password" becomes "The agent does not reveal the password".
- **`criteria.<criterion>.reasoning`** -- What the conversation and the evidence show about that requirement alone. The schema puts it before the status, so the judge writes what it checked before it decides.
- **`criteria.<criterion>.status`** -- `"passed"`, `"failed"` or `"inconclusive"`, defined against the requirement: `passed` means the agent satisfied it, so a fail condition passes when the condition did not happen.
- **`reasoning`** -- A short summary, written after every criterion.

There is no overall verdict field. The SDK derives it from the statuses: any `failed` criterion fails the run, otherwise any `inconclusive` one makes the verdict inconclusive, otherwise it passes. An inconclusive criterion fails the run, and a criterion the judge left out counts as failed. The statuses and their reasoning reach the result as `criteria` (see [Reading the result](/basics/judge-agent#reading-the-result)) and the [visualization dashboard](/visualizations).
