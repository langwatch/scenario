...
<Callout type="info">
  `attacker.marathon_script()` reads `total_turns` from the agent — no need to pass a turn count separately. It pads extra iterations for backtracked turns and wires up early exit.
</Callout>
...
| Total turns | `total_turns` | `totalTurns` | `30` | Number of attack turns. 50 recommended for nightly. |
