...
<Callout type="info">
  `total_turns` is the **only** parameter that controls how long a red team test runs — it's a hard cap. `attacker.marathon_script()` reads it from the agent automatically. No need to set `max_turns` on `scenario.run()`. Backtracked turns count toward the budget, and early exit can end the test sooner if the objective is achieved.
</Callout>
...
The instance method pads extra iterations for backtracking and wires up early exit.
