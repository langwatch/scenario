...
By default, tracing initializes lazily on the first `run()` call. If you need tracing active before that -- for example, to capture spans during test setup or agent warm-up -- call the setup function explicitly.
...
`setup_scenario_tracing()` (Python) and `setupScenarioTracing()` (TypeScript) accept the same options as the observability config. Once called, subsequent `run()` calls skip their own initialization.

<Callout type="info">
  If you configure observability in both your config file (`conftest.py` for Python, `scenario.config.js` for TypeScript) and via the explicit setup call, the explicit call wins -- `run()` will not re-initialize tracing.
</Callout>
...
Configuration is passed to `setup_scenario_tracing()` in `conftest.py`:
