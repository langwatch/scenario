...
[Pydantic AI](https://ai.pydantic.dev/) agents work seamlessly with Scenario through the <RefLink link={{ python: "agent_adapter.html#scenario.agent_adapter.AgentAdapter" }} code={{ python: "AgentAdapter" }} /> interface. The key is managing conversation history and handling the iterative agent execution.
