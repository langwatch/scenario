...
[Agno](https://github.com/agno-agi/agno) agents work seamlessly with Scenario through the <RefLink link={{ python: "agent_adapter.html#scenario.agent_adapter.AgentAdapter" }} code={{ python: "AgentAdapter"}} /> interface. The key is handling message state properly since Agno agents maintain their own conversation history.
