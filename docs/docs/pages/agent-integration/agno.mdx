...
[Agno](https://github.com/agno-agi/agno) agents integrate with Scenario through the <RefLink link={{ python: "agent_adapter.html#scenario.agent_adapter.AgentAdapter" }} code={{ python: "AgentAdapter"}} /> interface. Handle message state properly, as Agno agents maintain their own conversation history.
...
Integrate with Scenario as follows:
...
This conversion, however, only works for OpenAI agents. Messages coming from Gemini or Anthropic will have a slightly different format and will need more manual conversion for the tool calls.
