...
- **Session continuation** — per `threadId`, the first turn sends the full
  ...
  send only the new messages. If the CLI reports that a resumed session no
  longer exists, the adapter evicts it, warns via the injected `logger`, and
  re-runs the same turn from full history against a fresh session — so a
  single `call()` can spawn the CLI twice. Any other CLI failure — auth, rate
  limit, unknown model, a signal — rejects with `ClaudeCodeCliError` (see
  [Error handling](#error-handling)) and is not retried.
