...
```typescript
import { ClaudeCodeCliError } from "@langwatch/scenario";
...
try {
  ...
  if (cause instanceof ClaudeCodeCliError) {
...
:::warning `stderr` is unredacted CLI output

`stderr` carries whatever the CLI wrote to its error stream verbatim — it may
include sensitive detail (environment values, a rejected API key echoed in an
error). It is also an enumerable field, so it appears in
`JSON.stringify(error)` too. Do not log it verbatim to shared or public sinks;
redact or drop it before it leaves a trusted boundary.
