...
[OpenAI](https://openai.com/) agents integrate with Scenario through the <RefLink link={{ python: "agent_adapter.html#scenario.agent_adapter.AgentAdapter" }} code={{ python: "AgentAdapter" }} /> interface. OpenAI uses the standard message format, simplifying integration.
