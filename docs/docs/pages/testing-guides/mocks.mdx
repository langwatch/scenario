...
Mocks are simulated implementations of external dependencies (APIs, databases, tools, or services) that scenario tests use to ensure **deterministic, offline-friendly** execution.

:::tip
**Modern Agent Architecture**

Production agents typically use **tool calling** to interact with external systems instead of making direct API calls in their core logic. This separation of concerns makes agents more testable, maintainable, and reliable. Mock the tool execution functions rather than the agent's reasoning logic.
...
The following examples demonstrate different mocking levels, ordered by frequency of use in agent testing:
