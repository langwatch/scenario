...
**Also check:** `ragas/sql_query_equivalence` on [Evaluators on Scenarios](/advanced/evaluators) compares the SQL the agent ran against a golden query the scenario carries, so the equivalence check runs on the platform and reports next to the criteria.
