...
Tool calls are essential to modern agent workflows. This guide covers verifying tool usage, asserting on tool call behavior, and mocking tool responses for robust, deterministic tests.

## Checking for Tool Calls

Verify that your agent makes the correct tool call using the `state.has_tool_call("tool_name")` API in an assertion function. Place assertion functions directly in the `script` list as steps following the relevant agent or user turns.

**Example: Verify Weather Agent Calls the Weather API**
...
Verify not only that a tool was called, but also that it was called with the correct arguments.
...
const result = await scenario.run({
  ...
  script: [
    ...
    }), // JudgeAgent will evaluate based on criteria
  ],
});
```
...
## Grade a tool call with an evaluator

An evaluator grades a tool call after the run has its verdict and reports the result in LangWatch next to the criteria. Map its `output` input to the call, because a tool call is never inferred from the input name, and pass the question in `settings`.
...
result = await scenario.run(
    ...
    evaluators=[
        scenario.evaluator(
            ...
            required=False,
            mappings={"output": lambda state: state.tool_calls("get_current_weather").last.input},
            settings={
                "prompt": "Is the location argument a city, not a country? Answer true or false.",
            },
        ),
    ],
)
...
const result = await scenario.run({
  ...
  evaluators: [
    scenario.evaluator("langevals/llm_boolean", {
      required: false,
      mappings: { output: (state) => state.toolCalls("get_current_weather").last?.input },
      settings: {
        prompt: "Is the location argument a city, not a country? Answer true or false.",
      },
    }),
...
**Also check:** [Judge Criteria, Script Assertions and Evaluators](/testing-guides/judge-assertions-and-evaluators) for when a tool call check belongs in a script assertion, in the judge criteria or on an evaluator.
