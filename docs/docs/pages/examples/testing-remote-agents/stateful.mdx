...
import StatefulTestExample from "../../_generated/examples/testing-remote-agents-stateful.test.mdx";

# Stateful Pattern with Thread ID

Test agents that maintain conversation history server-side using thread identifiers. The adapter sends only the latest message plus a thread ID, and the server looks up the full history.
...
Stateful tests should verify the server maintains context:
...
- [JSON Pattern](/examples/testing-remote-agents/json) - For stateless agents
- [Domain-Driven TDD](/testing-guides/domain-driven-tdd) - Building stateful systems with TDD
