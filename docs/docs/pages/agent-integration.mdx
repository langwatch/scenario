...
Scenario supports any agent architecture through simple adapter patterns, making it completely framework-agnostic.
...
Agent `call` functions must receive the full chat history via `input.messages` to support message mocking in tests. Without this, Scenario cannot determine which messages belong to the agent, and mocking will fail.
:::
