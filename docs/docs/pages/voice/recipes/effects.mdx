...
In **Python**, per-step effect overrides via `scenario.user(audio_effects=[...])`
are on the roadmap but not yet in the stable API. Use adapter-level
`audio_effects` for global application, or pre-bake effects into WAV files and use
`scenario.audio()` for per-step control. See
[specs/voice-agents.feature §4.5](https://github.com/langwatch/scenario/blob/main/specs/voice-agents.feature)
