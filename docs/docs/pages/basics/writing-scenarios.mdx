...
// Additional assertions
if (!result.success) throw new Error("Scenario did not succeed");
if (result.messages.length < 4) throw new Error("Should have substantial conversation");
if (!result.messages.some(m => String(m.content).toLowerCase().includes("account"))) throw new Error("Should discuss account");
```
