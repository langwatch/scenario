...
<Callout type="info">
  All parameters are optional. When using [scripted simulations](/basics/scripted-simulations), prefer passing criteria inline to `scenario.judge(criteria=[...])` instead — see [Inline Criteria](/basics/scripted-simulations#scenariojudge). The judge uses global configuration defaults when parameters aren&apos;t specified.
</Callout>
