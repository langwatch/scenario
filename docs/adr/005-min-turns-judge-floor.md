...
7. **Startup validation: the floor must be a non-negative integer no greater than `maxTurns`**, checked when the scenario is built, not mid-run. Zero is legal and behaves like an unset floor. `minTurns == maxTurns` is also legal: every turn is gated for volunteered verdicts and the forced last-turn judgment (which always wins) delivers the only verdict.
...
| Validation | non-negative integer and `minTurns <= maxTurns`; zero is valid | At scenario build time |
...
- **v4 (2026-08-13, review)** — Defined and enforced the floor's input domain: a non-negative integer no greater than `maxTurns`, with zero valid. This prevents negative and non-finite no-op floors and fractional floors with undocumented rounding behavior.
